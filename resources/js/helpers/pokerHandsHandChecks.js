...
export default class HandChecks {
    ...
    singlePair(){
        const pairs = helpers.pairsCheck([...this.playersCards])
        if (pairs.length === 2) {
    ...
    tripsCheck(){
        ...
        if (firstRes.length < 3){
            const secondRes = helpers.getCardMatches(cards, 1) //if firstRes.length < 4, run check again with the shortened array
            if(secondRes.length < 3){
                const thirdRes = helpers.getCardMatches(cards, 1) //if firstRes.length < 4, run check again with the shortened array
    ...
    straightCheck(){
        ...
        const aceLowStraightRef = [...heirarchyOfCards].splice(0, 4);
    ...
    bookCheck(){
        const hasTrips = this.tripsCheck()
        if (hasTrips) {
            let tripsValue = hasTrips.highCard.card.slice(1), //remove items in the hand which match the trips value
            filteredTripsOut = [...this.playersCards].filter(card => card.slice(1) !== tripsValue)
    ...
    straightFlushCheck(){
        const straightCheck = this.straightCheck(this.playersCards);
    ...
    royalFlushCheck(){
        const firstCardsValue = helpers.sortCardsByValues([...this.playersCards])[0].value
